      ...
      <div className="py-6 sm:py-8 lg:py-12 px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto space-y-8">
         ...
         <div className="space-y-4">
           ...
           <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
             {[1, 2, 3].map(i => <div key={i} className="h-24 bg-card rounded-3xl animate-pulse" />)}
           </div>
         </div>
      </div>
    );
  ...
  if (error || !startup) {
    return (
      <div className="py-20 text-center flex flex-col items-center">
        ...
        <button onClick={() => router.back()} className="mt-4 rounded-full bg-foreground px-6 py-2 text-sm font-medium text-background">
          Go Back
        </button>
      </div>
    );
           ...
           <div className="bg-card border border-border rounded-3xl p-6">
             ...
             <div className="space-y-4">
               {startup.founderIds.map((fId) => (
                 <Link key={fId} href={`/founders/${fId}`} className="flex items-center gap-3 group">
                   ...
                   <div>
                     <p className="font-semibold text-sm group-hover:text-[#00C6D8] transition-colors">Founder Profile</p>
